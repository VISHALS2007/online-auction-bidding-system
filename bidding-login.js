// Bidding Login JavaScript functionality

// DOM Elements
const biddingLoginForm = document.getElementById('biddingLoginForm');
const emailInput = document.getElementById('email');
const passwordInput = document.getElementById('password');
const emailError = document.getElementById('emailError');
const passwordError = document.getElementById('passwordError');
const biddingBtn = document.querySelector('.bidding-btn');
...
// Form submission
biddingLoginForm.addEventListener('submit', function(e) {
    e.preventDefault();
    ...
    // Show loading state
    biddingBtn.classList.add('loading');
    biddingBtn.disabled = true;
    
    // Simulate API call
    setTimeout(() => {
        // Store user information with bidding context
        const userInfo = {
            email: user.email,
        ...
        showSuccessMessage('Bidding Portal');
        ...
        // Reset form
        biddingLoginForm.reset();
        clearError(emailInput, emailError);
        clearError(passwordInput, passwordError);
        
        // Remove loading state
        biddingBtn.classList.remove('loading');
...
function storeUserInfo(userInfo) {
    ...
    if (existingUserIndex !== -1) {
        ...
        users[existingUserIndex] = {
            ...
            lastLoginType: userInfo.loginType
        };
    } else {
        // Add new user
        users.push({
    ...
    const sessionInfo = {
        ...
        portalAccess: userInfo.portalAccess
    };
    localStorage.setItem('session', JSON.stringify(sessionInfo));
}

// Success message
function showSuccessMessage(portalType) {
    ...
    notification.innerHTML = `
        <div style="display: flex; align-items: center; gap: 8px;">
            ...
            <span>Successfully logged into ${portalType}! Redirecting...</span>
        </div>
    `;
